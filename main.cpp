...
int main()
{
    ...
    std::set<std::string> exclude_strings;
    ...
    //remove all the excludes strings
    for (auto it = term_docs_positions.begin(); it != term_docs_positions.end();)
    {
        if (exclude_strings.find(it->first) != exclude_strings.end())
            it = term_docs_positions.erase(it);
        else
            ++it;
    }
